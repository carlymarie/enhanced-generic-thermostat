...
DEFAULT_NAME = "Enhanced Generic Thermostat"
...
class GenericThermostat(ClimateEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self):
        ...
        if self._keep_alive:
            self.async_on_remove(
                async_track_time_interval(
                    self.hass, self._async_control_cooling, self._keep_alive
                )
            )
        ...
        async with self._temp_lock:
            ...
            if self._is_device_active:
                ...
                if (self.ac_mode and too_hot) or (not self.ac_mode and too_cold):
                    if self.ac_mode:
                        _LOGGER.info("Turning on cooler %s",
                                     self.cooler_entity_id)
                    else:
    ...
    def _is_device_active(self):
        ...
        if not self.hass.states.get(self.heater_entity_id) or not self.hass.states.get(self.cooler_entity_id):
            return None
